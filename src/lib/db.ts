...
async function ensureSchema(db: Client) {
  await db.executeMultiple(`
    ...
    /* Pilot board — initiatives the team proposes for the pilot. */
    CREATE TABLE IF NOT EXISTS pilot_initiatives (
      id TEXT PRIMARY KEY,
      title TEXT NOT NULL,
      description TEXT NOT NULL DEFAULT '',
      author_role TEXT,
    ...
    /* Per (initiative × stage) gaps — what's missing to run the
     * initiative through that stage. Cascading delete from initiative. */
    CREATE TABLE IF NOT EXISTS pilot_gaps (
      id TEXT PRIMARY KEY,
      initiative_id TEXT NOT NULL REFERENCES pilot_initiatives(id) ON DELETE CASCADE,
