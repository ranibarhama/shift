...
import { listProcesses, getStagesForProcess, getEdgesForProcess, getItemsForStages } from "./queries";
...
export function loadOverview(): OverviewData {
  ...
  for (const p of processes) {
    const s = getStagesForProcess(p.id);
    stagesByProcess[p.id] = s;
    edgesByProcess[p.id] = getEdgesForProcess(p.id);
    allStageIds.push(...s.map((x) => x.id));
  }
  const items = getItemsForStages(allStageIds);
